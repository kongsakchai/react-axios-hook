...
const useGet = <T>(
  url: string,
  options?: Options<T>
): [T | undefined, boolean, Error | undefined, CallFunction<T>] => {
  ...
  async function callFunction(
    sendOption?: SendDataOptions
  ): Promise<T | undefined>;
  async function callFunction(
    sendData?: string | SendDataOptions
  ): Promise<T | undefined> {
    setLoading(true);
    const params =
      typeof sendData == "string" ? sendData : sendData?.params ?? "";
