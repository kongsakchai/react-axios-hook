...
import { CallFunction, Options, SendDataOptions } from ".";

const usePost = <T>(url: string, options?: Options<T>): [T | undefined,boolean, Error | undefined, CallFunction<T>] => {
    ...
    async function callFunction(sendData?: string | SendDataOptions): Promise<T | undefined > {
        ...
        const data = (typeof sendData == 'string') ? undefined : (sendData?.data)

        const resData = await axios.post(url + params, data).then((res) => {
            setResult(res.data);
            return res.data;
        }).catch((e) => {
